...
describe('hook util suite', () => {
  ...
  test("transform is able to take a 'family' from a valid gfont url", () => {
    ...
    let matcher = '';
    for (const l of 'test') {
      let char = String.fromCharCode(l.charCodeAt(0) + 1);
      while (!SAFE_CHAR_PATTERN.test(char)) {
        char = String.fromCharCode(l.charCodeAt(0) - 1);
      }
      matcher += char;
    }
  ...
  test('transform throws an error if family cant be discerned and isnt supplied', () => {
    ...
    expect(() => {
      transform(urlPreHash);
    }).toThrow();
    expect(() => {
      transform(urlPreHash);
    }).toThrowError('MISSING FONT FAMILY');
  });
  test('createLinkElement - string', () => {
    ...
    expect(linkEl).toHaveAttribute('data-fonturlhashed', transform(stubHref));
  });
  ...
  test('checkDOMTreeForLinkEl - string - false', () => {
    ...
    expect(
      checkDOMTreeForLinkEl('http://someothersite.com/css?family=notstub:300')
    ).toBeFalsy();
  });
