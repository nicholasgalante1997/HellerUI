...
describe('Container', () => {
  ...
  test('background color', () => {
    const { container } = render(<Container height={100} width={200} background="red" />);
    expect(container.firstChild).toHaveStyle('background: red;');
  });
  test('background image', () => {
    const url = 'https://placedog.net/500';
    const { container } = render(<Container height={100} width={200} image={url} />);
  ...
  test('onClick', () => {
    ...
    const { container } = render(<Container onClick={mockFn} height={100} width={200} />);
